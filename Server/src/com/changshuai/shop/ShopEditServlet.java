...
public class ShopEditServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (ServletFileUpload.isMultipartContent(req)) {
			...
			while (iter != null && iter.hasNext()) {
				...
				if (item.isFormField()) {
					...
					if (item.getFieldName().equals("shopName")) {
						shop.setShop_name(str);
					} else if (item.getFieldName().equals("description")) {
						shop.setShop_description(str);
					} else if (item.getFieldName().equals("price")) {
						shop.setShop_price(str);
					} else if (item.getFieldName().equals("discountPrice")) {
						shop.setShop_discount_price(str);
					} 
				}
				// 文件域
			...
			shop.setShop_pic(fileName);
			shopMgrDao.save(shop);
			req.setAttribute("s", "0");
			req.getRequestDispatcher("/shopsEdit.jsp").forward(req, resp);
		}
