...
public class PeripheralFoodDao {
    private MySQLiteOpenHelper mySQLiteOpenHelper;

    public PeripheralFoodDao(Context context) {
    ...
    public boolean add(String name , String address, String context, String telephone){
        ...
        //组拼sql语句实现的
        long result = db.insert("peripheralfood", null, values);
        db.close();
        if(result==-1){
            return false;
        }else{
    ...
    public int delete(String name){
        ...
        int result = db.delete("peripheralfood", "name = ?", new String[]{name});
        db.close();
        return result;
    }
...
//        Cursor cursor = db.query("food", new String[]{"phone"}, "name=?", new String[]{name}, null, null, null);
...
//    }
    ...
    public List<PeripheralShop> findAll(){
        ...
        SQLiteDatabase db = mySQLiteOpenHelper.getReadableDatabase();
        Cursor cursor =  db.query("peripheralfood", null, null, null, null, null, null);
